...
class jc_com_afm extends JCommentsPlugin
{
	function getObjectTitle($id)
	{
		...
		$db->setQuery( 'SELECT file_title FROM #__afm_files WHERE published = 1 AND file_id = ' . $id );
	...
	function getObjectLink($id)
	{
		...
		$db = & JCommentsFactory::getDBO();
		$db->setQuery( 'SELECT cat_id FROM #__afm_files WHERE published = 1 AND file_id = ' . $id );
		$catid = $db->loadResult();
