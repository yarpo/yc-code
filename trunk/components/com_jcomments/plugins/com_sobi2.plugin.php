...
class jc_com_sobi2 extends JCommentsPlugin
{
	function getObjectTitle($id)
	{
		...
		$db->setQuery( 'SELECT title FROM #__sobi2_item WHERE published = 1 AND itemid = ' . $id );
		$title = $db->loadResult();
	...
	function getObjectLink($id)
	{
		...
		if (!isset($GLOBALS['jc_sobi2itemid'])) {
		        ...
		        $_requestOption = JCommentsInput::getParam($_REQUEST, 'option', '');
		        $_requestItemid = (int) JCommentsInput::getParam($_REQUEST, 'Itemid', 0);
			$_Itemid = null;
