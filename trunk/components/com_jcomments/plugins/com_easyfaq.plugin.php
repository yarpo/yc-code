...
class jc_com_easyfaq extends JCommentsPlugin
{
	function getTitles($ids)
	{
		...
		$db->setQuery( 'SELECT id, title FROM #__easyfaq WHERE published = 1 AND IN (' . implode(',', $ids) . ')' );
	...
	{
		$db = & JCommentsFactory::getDBO();
		$db->setQuery( 'SELECT title FROM #__easyfaq WHERE published = 1 AND id = ' . $id );
	...
	{
		$db = & JCommentsFactory::getDBO();
		$db->setQuery( 'SELECT created_by FROM #__easyfaq WHERE id = ' . $id );
