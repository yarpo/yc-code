...
class jc_com_jportfolio extends JCommentsPlugin
{
	function getObjectTitle($id)
	{
		...
		$db->setQuery( 'SELECT name FROM #__jportfolio_projects WHERE published = 1 AND id = ' . $id );
	...
	function getObjectLink($id)
	{
		...
		$db = & JCommentsFactory::getDBO();
		$db->setQuery( 'SELECT catid FROM #__jportfolio_projects WHERE published = 1 AND id = ' . $id );
		$catid = $db->loadResult();
