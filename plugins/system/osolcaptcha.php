...
");
					...
					case 'com_user':
						switch($view)
						{
							...
							case 'remind':
								...
								break;
							case 'reset':
								if($isEnabledForForm["enableForReset"])
								{
	...
	function mailBotScoutResult()
	{
				...
				$message = "Following request from IP:$XIP returned a -ve result on botscout verification in ".JURI::current()."\r\n Get vars =".var_export($_GET,true)."\r\n POST vars =".var_export($_POST,true)."\r\n REQUEST vars =".var_export($_REQUEST,true);
				//echo $subject."\r\n".$message ;exit;
